...
use crate::assembler_state::{AssemblerState, OutputFile};
...
pub fn materialize_prebuild_sources(
    ...
    remove_invented_assembled_dir(new_state)?;
...
fn remove_invented_assembled_dir(state: &AssemblerState) -> io::Result<()> {
    let assembled_dir = state
        .project_root
        .join("prebuild")
        .join("src")
        .join("assembled");
    ...
    if assembled_dir.exists() {
        fs::remove_dir_all(assembled_dir)?;
    }
...
#[allow(dead_code)]
fn _assert_output_file_is_used(_: &OutputFile) {}
